...
use Jamiezyx\threevsthree\model\Arena;
use Jamiezyx\threevsthree\utils\PluginUtils;
use Jamiezyx\threevsthree\model\SignRefreshTask;
use Jamiezyx\threevsthree\ThreeVsThree;
...
class ArenaManager{
	...
	public function init(Config $config){
		...
		$task = new SignRefreshTask(threevsthree::getInstance());
	...
	public function parseArenaPositions(array $arenaPositions) {
		foreach ($arenaPositions as $n => $arenaPosition) {
			...
			if(($level = Server::getInstance()->getLevelByName($arenaPosition[3])) === null){
				Server::getInstance()->getLogger()->error("[3vs3] - " . $arenaPosition[3] . " is not loaded. Arena " . $n . " is disabled.");
			}
			else{
				...
				Server::getInstance()->getLogger()->debug("[3vs3] - Arena " . $n . " loaded at position " . $newArenaPosition->__toString());
			}
		}
	}	
		...
		foreach ($signPositions as $n => $signPosition) {
			...
			if(($level = Server::getInstance()->getLevelByName($signPosition[3])) !== null){
				...
				if($tile != null){
					...
					$cleanthreevsthreeTitle = TextFormat::clean(threevsthree::SIGN_TITLE);
					
					// Load it only if it's a sign with threevsthree title
					if($tile !== null && $tile instanceof Sign && $cleanTileTitle === $cleanthreevsthreeTitle){
	...
	public function addNewPlayerToQueue(Player $newPlayer){
		...
		if(in_array($newPlayer, $this->queue)){
			PluginUtils::sendDefaultMessage($newPlayer, threevsthree::getMessage("queue_alreadyinqueue"));
			return;
		}
		...
		if($currentArena != null){
			PluginUtils::sendDefaultMessage($newPlayer, threevsthree::getMessage("arena_alreadyinarena"));
			return;
		}
		...
		// display some stats
		PluginUtils::logOnConsole("[3vs3] - There is actually " . count($this->queue) . " players in the queue");
		PluginUtils::sendDefaultMessage($newPlayer, threevsthree::getMessage("queue_join"));
		PluginUtils::sendDefaultMessage($newPlayer, threevsthree::getMessage("queue_playersinqueue"). count($this->queue));
		$newPlayer->sendTip(threevsthree::getMessage("queue_popup"));
	...
	private function launchNewRounds(){
		...
		// Check that there is at least 6 players in the queue
		if(count($this->queue) < 6){
		...
		if($freeArena == NULL){
			Server::getInstance()->getLogger()->debug("[3vs3] - No free arena found");
			return;
		}
		...
		Server::getInstance()->getLogger()->debug("[3vs3] - Starting duel : " . $roundPlayers[0]->getName() . " vs " . $roundPlayers[1]->getName());
	...
	/**
	 * Refresh all 3vs3 signs
	 */
	public function refreshSigns(){
		foreach ($this->signTiles as $signTile){
			if($signTile->level != null){
				
				$signTile->setText(threevsthree::SIGN_TITLE, "-Waiting " . $this->getNumberOfPlayersInQueue(), "-Arenas: " . $this->getNumberOfFreeArenas(), "-+===+-");
			}
		}
	}
}
